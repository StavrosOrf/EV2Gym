...
if __name__ == "__main__":
    ...
    file_name = f"{problem}_{trajecotries_type}_{number_of_charging_stations}_cs_{n_transformers}_tr_{steps}_steps_{timescale}_timescale_{n_trajectories}_trajectories3.pkl"
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(tqdm(executor.map(run_trajectory, range(n_trajectories)), total=n_trajectories))
